document.addEventListener('DOMContentLoaded', function() {
    setTimeout(async function() {
        ...
        try {
            const {
                db, collection, getDocs, query, orderBy, addDoc, deleteDoc, doc, writeBatch
            ...
            async function addCarouselItem() {
                ...
                try {
                    ...
                    const newItem = { title, type, mediaUrl, order: carouselItems.length };
            ...
            function initializeSortable() {
                ...
                if (typeof Sortable === 'undefined') {
                    console.error('La librairie SortableJS n\'est pas chargée.');
                    return;
                }
                new Sortable(el, {
                    animation: 150,
                    ...
                    onEnd: async function() {
                        ...
                        const newOrder = items.map((item, index) => ({ id: item.dataset.id, order: index }));
                        try {
                            const batch = writeBatch(db);
